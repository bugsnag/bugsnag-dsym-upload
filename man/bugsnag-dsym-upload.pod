...
Given a directory, bugsnag-dsym-upload uploads any *.dSYM bundles within the
directory, and recursive subdirectories to the Bugsnag Upload API (https://upload.bugsnag.com).
As a pre-upload step, bitcode symbol maps can be combined with dSYM files to ensure
...
=item --project-root DIR

The root directory of the project. This will help to group error reports by project
